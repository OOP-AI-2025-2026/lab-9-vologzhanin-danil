...
public class Task {
    ...
    public void rearrange(Queue<Integer> q) {
        ...
        // 1. Непарні -> Черга (на початок), Парні -> Стек
        ...
        // 2. Парні (зі стека) -> Кінець Черги
        // Отримуємо: [Непарні] [Парні у зворотному порядку]
        while (!s.isEmpty()) {
            q.add(s.pop());
        }

        // 3. Відновлення порядку парних
        // Кількість парних: initialSize - q.size() (непарні)
        int evens = initialSize - q.size();

        // Парні зараз у кінці, переносимо їх на початок
        ...
        // 4. Переносимо непарні на кінець
